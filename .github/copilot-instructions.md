...
112. **`ns-cloud-a*.googledomains.com` nameservers = Squarespace Domains, NOT Google** — Google sold Google Domains to Squarespace in 2023. The nameserver hostnames kept the `googledomains.com` suffix, so `dig {{DOMAIN}} NS` looks like Google DNS — it is not. **All DNS records (CNAME for ACM cert validation, DKIM records for SES, CNAME for CloudFront www subdomain) must be added at [domains.squarespace.com](https://domains.squarespace.com) → DNS.** Not Google Cloud DNS. Not squarespace.com/config (that's the website builder). When adding CNAMEs: omit the domain suffix in the Host field — Squarespace appends it automatically. This mistake has caused agents to direct users to the wrong portal for ACM validation, SES DKIM, and CloudFront CNAME setup.
